...
export default function SetupPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const teachingStyle = (form.elements.namedItem('teachingStyle') as HTMLSelectElement).value
    ...
    const res = await fetch('/api/admin/users', {
      ...
      body: JSON.stringify({ username, displayName, password, level, teachingStyle }),
    })
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input name="displayName" type="text" required placeholder="e.g. Dad"
              className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Spanish Level</label>
          ...
          <div>
            ...
            <select name="teachingStyle" defaultValue="CONVERSATION"
              className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400">
              <option value="CONVERSATION">Conversation — natural chat, gentle corrections</option>
              <option value="DRILLS">Drills — one grammar pattern per turn, 2-3 sentences from you</option>
              <option value="IMMERSION">Immersion — mostly Spanish, push your level</option>
            </select>
          </div>
